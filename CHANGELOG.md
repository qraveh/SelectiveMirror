...
- **BUG-R3-1 (Medium)** — `.syncignore` parent-exclusion does not block child negation. `foo/**` followed by `!foo/bar/baz.txt` re-includes the leaf, contradicting the gitignore spec ("It is not possible to re-include a file if a parent directory of that file is excluded"). Documented deviation in `internal/filter/filter.go::IsExcluded`. Test: `system-validation/TestPanelR3_Gitignore_ExcludedParentBlocksChildNegation`.
- **BUG-R4-1 (High)** — Concurrent `addmirror` from two terminals can erase a pre-existing mirror via read-modify-write race in `internal/config/edit.go`. Workaround: edit `config.yaml` from a single shell at a time. Fix planned via OS-level file lock + atomic-rename, reusing the `internal/lock` primitive. Test: `system-validation/TestPanelR4_CLI_ConcurrentAddMirror`.
