// SM Symlink-asymmetry Medium close-out (v1.0.1).
//
// Background: pre-v1.0.1, foreground mode followed symlinks while
...
// Service mode closed this in v0.9.x via SEC-H5; v1.0.1 aligns
// foreground.
//
...
// TestSymlinkAsymmetry_ForegroundDefaultsToReject is the v1.0.1 close-
...
func TestSymlinkAsymmetry_ForegroundDefaultsToReject(t *testing.T) {
	...
	if !wantFG.Match(src) {
		t.Errorf("cmd/smirror/main.go MUST set "+
			...
			"in the foreground startup path (the v1.0.1 close-out of "+
			"the Symlink-asymmetry Medium). Without this, foreground "+
			"mode reopens the pre-v1.0.1 default-follow behavior "+
...
func TestSymlinkAsymmetry_ConfigFieldExists(t *testing.T) {
	...
	if !want.Match(src) {
		t.Errorf("internal/config/config.go::Global MUST declare " +
			...
			"v1.0.1 Symlink-asymmetry close-out to compile and to " +
