...
The MSI installer is built using WiX v6 (`WixToolset.Sdk/6.0.2`). The WiX source files define:
...
Build it with `installer\build-msi.ps1` (see *Building from Source* above). The output filename is `SelectiveMirror.msi` -- the version goes into the MSI's `ProductVersion` field rather than the filename. To read it back, use the helper described next.

## Inspecting an MSI

`scripts\msi-info.ps1` reads an MSI's metadata (Product Name / Version / Code, Upgrade Code, Manufacturer, plus file size and SHA-256) without installing it. Read-only, no admin required -- it opens the MSI's internal database via the `WindowsInstaller` COM API and queries the `Property` table.

```powershell
# Default formatted output
powershell -ExecutionPolicy Bypass -File scripts\msi-info.ps1 `
    -Path installer\bin\Release\SelectiveMirror.msi
# ProductName    : SelectiveMirror
# ProductVersion : 1.0.18
# ProductCode    : {EE45728C-...}
# UpgradeCode    : {7E3A9F12-...}
# ...

# Pipeline mode -- bare value on stdout
$v = & powershell -ExecutionPolicy Bypass -File scripts\msi-info.ps1 `
        -Path .\SelectiveMirror.msi -Property ProductVersion
# $v = '1.0.18'

# JSON for CI / downstream tools
powershell -ExecutionPolicy Bypass -File scripts\msi-info.ps1 `
    -Path .\SelectiveMirror.msi -Json | ConvertFrom-Json
```

**Exit codes**: `0` success, `1` general failure (e.g. file not an MSI), `2` requested `-Property` not present in the table.

Typical uses:
- Confirm a freshly built MSI before publishing (`ProductVersion` matches source).
- CI gate: assert built MSI version matches the git tag.
- Upgrade-path sanity check: compare two MSIs -- same `UpgradeCode`, bumped `ProductVersion`, new `ProductCode` means in-place upgrade will work.

`Get-Help scripts\msi-info.ps1 -Full` lists every parameter and example.
...
- `SelectiveMirror.msi` -- MSI installer (version inside, not in filename; see *Inspecting an MSI* above)
