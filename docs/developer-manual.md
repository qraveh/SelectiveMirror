...
| `telemetry` | `internal/telemetry/` | Three-tier opt-in consent (None/Standard/Reliability), tier-gated startup update check, shared report sanitizer (bug-report + crash), version-derived HMAC signer, durable disk queue, length-first JSONB-canonical JSON. Server-side: stream-aggregate-and-discard (no personal data on disk by construction); Supabase RPC + Cloudflare Worker proxy. Pending: `smirror telemetry` runtime CLI (SM-157), report-bug --submit pipeline (SM-158), MSI custom dialog UI (SM-168). See `docs/telemetry-architecture-v2.md`, `docs/PRIVACY.md`, and `docs/cli-telemetry-command.md`. |
