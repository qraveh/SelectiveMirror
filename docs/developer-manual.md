...
| `telemetry` | `internal/telemetry/` | Three-tier opt-in consent (None/Standard/Reliability), tier-gated startup update check, shared report sanitizer (bug-report + crash), version-derived HMAC signer (SM-168, shipped pre-round-2), length-first JSONB-canonical JSON, classifier (`classify.go`) + Contribute() HTTP client (`contribute.go`). Server-side: stream-aggregate-and-discard (no personal data on disk by construction); Supabase RPC + Cloudflare Worker proxy. Shipped: `smirror telemetry` runtime CLI (SM-157, 0.9.83-dev), `report-bug --submit` pipeline (SM-158, 0.9.89-dev). **Deferred to v1.0.x:** install-event submit pipeline — `first_seen`/`upgrade`/`reliability_snapshot` payloads have inspect-only generators today; no startup/upgrade-detection writer calls `Contribute()` for them (FINDING 16, round-5 validation memo 2026-05-03). The `queue.go` durable-on-disk queue is also deferred-but-staged: tests + atomic semantics are in tree, but no production caller — see the package docstring's STATUS note. See `docs/telemetry-architecture-v2.md`, `docs/PRIVACY.md` "Currently shipped vs. deferred" table, and `docs/cli-telemetry-command.md`. |
