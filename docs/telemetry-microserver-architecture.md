...
### Maintainer reporting layer (added 2026-04-28 after panel review)

For a single-maintainer OSS project at low volume, dashboards are the wrong tool. Instead, two layers serve the maintainer:

1. **SQL views** for ad-hoc Supabase Studio queries: `bug_report_human` (denormalized one-row-per-report with a 500-char preview), `bug_report_clusters` (signature recurrence, last 180 days), `install_summary` (per-install snapshot), `weekly_health` (single-row dashboard). See `docs/telemetry-views.sql`. Service-role-only; no anon access.

2. **Weekly Markdown digest** generated by GitHub Action every Sunday at 03:00 UTC. The Action runs `scripts/telemetry-report.py` against the live database, renders to Markdown, opens a PR adding `docs/telemetry/weekly-YYYY-WWNN.md`. The PR is the notification mechanism. Form chosen because:
   - Renders natively on GitHub (read on mobile)
   - Version-controlled (the `git log` is a longitudinal view)
   - Zero hosting cost
   - Degrades gracefully: when n is too small for analysis, the report explicitly says so rather than fabricating trends

   Anti-patterns explicitly avoided: vanity counters (total installs ever), pie charts at n<10, MAU/DAU (we don't have heartbeats), traffic-light status boards, fake geo data.

   See `docs/operations/telemetry-ops.md` for the maintainer's runbook.
