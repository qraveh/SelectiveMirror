...
## Status board (refreshed 2026-05-26 for v1.0.60 release — post Wacatac.B!ml recovery; v1.0.59 draft withdrawn; SM-218/SM-219/SM-220/SM-221 + R-24/R-25 + Open Mediums close-out)
...
| **Open Mediums** | ≤ 3 open, all with planned fix versions | **4 open** (2 closed in v1.0.60 prep): ~~addmirror fresh-config file mode~~ (closed; the SEC-H6 fix is in place + the regression test reasoning corrected to reflect Windows-NTFS ACL is the real protection — see CHANGELOG [Unreleased]); ~~foreground symlink-follow~~ (closed via `config.AllowSymlinks` default-reject; foreground now mirrors service-mode SEC-H5 default); remaining: CLAIMS-MAP HMAC timing benchmark (v1.0.x), CLAIMS-MAP pg_stat_statements smoke (v1.0.x), SM-082 items 3+4 (svc.Control inconsistency + Anomaly Detail stderr; v1.0.x), SM-057 (burst-delete reconcile sleep; v1.0.x). Approaching the ≤ 3 target; 🟡 until the remaining 4 close in next release / v1.0.x. | 🟡 |
| **Telemetry health (continuous live measurement)** | Cloudflare Worker access-log probe daily-green; envelope rate steady; zero None-tier records over None-tier installs (NFR-PR-01 target = 0.000) | **Live + functional end-to-end as of 2026-05-21.** Major v1.0.60-prep discovery: SM-219 — HMAC master-key derivation mismatch between `verify_versioned_hmac` SQL and release.yml + binary — caused EVERY CI-built smirror's `first_seen` and `upgrade` event to be silently rejected by the Worker since the install-event pipeline shipped (0.9.102-dev). Rollup tables stayed at 0 rows through the entire v1.0.0 release cycle as a consequence. SM-219 SQL migration deployed to live Supabase 2026-05-21; a CI-style local-built v1.0.23 binary then landed the project's first real `first_seen` and upgrade events. Worker emits records, schema-validated daily by `.github/workflows/telemetry-emulation.yml`, CLAIMS-MAP gate at 25/28 GREEN, fingerprint probe (cf-ray + SM Worker custom header) verified per-tag. R-12 NFR-PR-01 first ratio for v1.0.60: `0/0` vacuous (the v1.0.0→v1.0.60 window had zero real-user contributions; the pre-SM-219 portion of the window had zero contributions period; maintainer-side validation rows from SM-219 / SM-220 verification have been removed). First quantified non-vacuous ratio will appear in v1.0.60→next release release notes once real-user installs of v1.0.60 (the first publicly-released telemetry-functional version — v1.0.59 was withdrawn before publication due to the Wacatac.B!ml FP) accumulate. | 🟢 |
...
| **Docs vs. code drift** | All `docs/*.md` reference real file paths and current behaviors | Latest sweep 2026-05-26 (this v1.0.60-release refresh, including v1.0.59→v1.0.60 label sweep across iso-compliance + this dashboard after the v1.0.59 draft withdrawal). v1.0.60 R-16 audit-doc closure: `docs/iso-compliance.md` updated in 7 places to mark A-29119-01 closed (Test Strategy doc `docs/test-strategy.md` was authored 2026-05-03; the audit doc now consistently reflects that closure). Earlier 2026-05-14 path-move sweep (`C:\SelectiveMirror\` → `C:\mine\SelectiveMirror\`) landed via v1.0.59-prep housekeeping commit (12 files). | 🟢 |
...
- **Why it's red**: No certificate yet. SignPath Foundation rejected the v1.0.0 OSS application on reputation grounds (2026-05-21 memo). Every install triggers SmartScreen until the cert lands, AND cloud Defender ML continues to flag fresh unsigned Go binaries (see Wacatac episode below).
- **Wacatac.B!ml episode (v1.0.59 + v1.0.60)**: Microsoft Defender's ML classifier flagged the v1.0.59 MSI as `Trojan:Win32/Wacatac.B!ml` on first download — a false positive driven by ML signals against unsigned anonymous Go binaries (no PE VERSIONINFO + stripped symbol table looked statistically like packed/obfuscated malware). v1.0.59 draft was withdrawn before public release. **R-24 (PE VERSIONINFO embedding via `goversioninfo`)** and **R-25 (`-s -w` removed from ldflags so the symbol table stays in)** landed for v1.0.60 as the source-side ML mitigations. Local-Defender scan of v1.0.60 build (`MpCmdRun.exe -Scan -ScanType 3 -File ...`) returns `found no threats` — but cloud Defender (which uses fresher ML models + reputation signals + download context) **still flagged v1.0.60 on first user download** with the same ThreatID. R-24 + R-25 reduced FP rate (defeated local ML) but didn't close the cloud ML + cold-reputation combo. WDSI submission for the v1.0.60 SHA is the right immediate response (24-72h reclassification turnaround) but is per-SHA — every fresh unsigned release will face the same gauntlet.
- **Remediation (revised 2026-05-26)**: see [memory/signing_provider_update_2026-05-26.md](../../../Users/raveh/.claude/projects/C--mine-SelectiveMirror/memory/signing_provider_update_2026-05-26.md) for the full corrected matrix. Summary: Azure Trusted Signing is **unavailable for Israeli individual subscribers** (the 2026-05-21 memo's "open question, loosening, needs confirmation" framing was optimistic; reality on 2026-05-26: orgs USA/Canada/EU/UK with ≥3yr verifiable history, individuals USA/Canada with onboarding paused). SSL.com is **not free** — the earlier "free OSS" claim conflated SSL.com with SignPath / OSSign. Revised plan: (a) apply to **OSSign** (free, criteria unverified, low cost to try); (b) procure **SSL.com OV** (~$249/year commercial primary, validates via business registration + phone callback, supports cloud-CI signing via SSL.com's cloud HSM); (c) **Sectigo OV** (~$200/yr) as commercial alternative; (d) **Certum Open Source** (~€30/yr, USB token, breaks all-cloud CI — would push signing step out of GitHub Actions into a manual local-machine step) as last resort; (e) reapply to SignPath Foundation in 6-12 months once reputation accumulates. R-24 + R-25 reduce FP rate at the local-ML layer; signing closes the rest. release.yml insertion points are provider-agnostic (lines 297-322 for smirror.exe, 431-460 for MSI).
- **Owner**: maintainer.
