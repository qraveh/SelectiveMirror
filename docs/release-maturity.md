...
| **Branch CI (`ci.yml`) status** | Latest run on `master` is green | Pre-existing `golangci-lint` debt (9 findings in `internal/hooks/`, `internal/lock/`, `internal/sync/`, `internal/config/edit_test.go`, `internal/sync/liveness.go`) has been red on `master` for multiple commits. Independent of `release.yml` (which uses `go vet` + tests, not lint) — release tags can ship while `ci.yml` is red. Tracked here so dashboard readers see it. **Owner: maintainer; cleanup commit pending (PR-PRE-D4 follow-up).** | 🔴 |
