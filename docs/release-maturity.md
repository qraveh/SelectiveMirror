...
- **Why it's red**: No certificate yet. SignPath Foundation rejected the v1.0.0 OSS application on reputation grounds (2026-05-21 memo); pivot to Azure Trusted Signing (~$10/mo commercial, primary) with SSL.com OSS and Certum OSS as fallbacks. Every install triggers SmartScreen until the cert lands.
- **Wacatac.B!ml episode (v1.0.59 → v1.0.60)**: Microsoft Defender's ML classifier flagged the v1.0.59 MSI as `Trojan:Win32/Wacatac.B!ml` on first download — a false positive driven by ML signals against unsigned anonymous Go binaries (no PE VERSIONINFO + stripped symbol table looked statistically like packed/obfuscated malware). v1.0.59 draft was withdrawn before public release. **R-24 (PE VERSIONINFO embedding via `goversioninfo`)** and **R-25 (`-s -w` removed from ldflags so the symbol table stays in)** landed for v1.0.60 as the source-side ML mitigations. Local-Defender scan of v1.0.60 build (`MpCmdRun.exe -Scan -ScanType 3 -File ...`) returns `found no threats` — vs the Wacatac flag on v1.0.59. WDSI submission template at `docs/operations/wdsi-fp-submission-v1.0.59.md` keeps the v1.0.59 SHA on Microsoft's reclassification queue regardless of withdrawal. Both R-24 and R-25 are partial mitigations only — a real Authenticode signature is still the primary remediation.
- **Remediation**: Procure Azure Trusted Signing certificate (~1-2 week provisioning). Integrate the Trusted Signing GitHub Action between MSI build and upload (post-build, pre-attestation, pre-upload). README + SECURITY.md reference the plan; release.yml already has insertion-point comments. R-24 + R-25 reduce FP rate at the ML layer; signing closes the rest.
- **Owner**: maintainer.
